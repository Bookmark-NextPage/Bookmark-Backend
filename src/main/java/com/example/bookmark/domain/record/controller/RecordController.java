...
public class RecordController {
    ...
    @PostMapping("/{collectBookId}/chapters/{chapterId}/records")
    public ResponseEntity<ApiResponse<RecordSaveResponse>> createCustomRecord(
        ...
        RecordSaveResponse response = recordService.createCustomRecord(userId, collectBookId, chapterId, request);
        return ResponseEntity.ok(ApiResponse.onSuccess(response));
    }

    @Operation(summary = "콜렉트북에 기록 생성 - 메모지 기반O", description = "버킷 보드에서 '이뤘어요' 버튼을 눌러 작성일 기준 월별 자동 생성 콜렉트북 챕터에 최종 저장합니다.")
    @PostMapping("/memos/{memoId}/records")
    public ResponseEntity<ApiResponse<RecordSaveResponse>> createMemoRecord(
            @LoginUserId Long userId,
            @PathVariable Long memoId,
            @Valid @RequestBody RecordCreateRequest request
    ) {
        RecordSaveResponse response = recordService.createMemoRecord(userId, memoId, request);
        return ResponseEntity.ok(ApiResponse.onSuccess(response));
    }
    ...
    @GetMapping("/{collectBookId}/records/{recordId}")
    public ResponseEntity<ApiResponse<RecordDetailResponse>> getRecordDetail(
            @LoginUserId Long userId,
            @PathVariable Long collectBookId,
            @PathVariable Long recordId
    ) {
        RecordDetailResponse response = recordService.getRecordDetail(userId, collectBookId, recordId);
        return ResponseEntity.ok(ApiResponse.onSuccess(response));
    }

    @Operation(summary = "댓글 작성", description = "친구인 유저들끼리 서로의 콜렉트북 내 기록에 댓글을 남깁니다.")
    @PostMapping("/{collectBookId}/records/{recordId}/comments")
    ...
    @PostMapping("/{collectBookId}/records/{recordId}/likes")
    ...
    @DeleteMapping("/{collectBookId}/records/{recordId}/likes")
